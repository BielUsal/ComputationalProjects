...
md"""
# Numerically Solving the Kronig-Penney 
...
However, we wiull apply Bloch's theorem, which states that, since our potential will be periodic, we will have that:
$$\psi(x) = e^{iqx} u(x)_{nq}$$

Where $$u$$ is a periodic function. This leads to the equation for u:

$$-\frac{\hbar^2}{2m} \partial^2_x u_{nq} - \frac{i\hbar^2q}{m} \partial_x u_{nq} +V(x) u_{nq} = \left(E-\frac{\hbar^2 q^2}{2m}\right)u_{nq}$$
And therefore:

$$\frac{\hbar^2}{2m dx^2} \left(-u[i+1] -  u[i-1]\right) + u[i]\left(V[i] +2 \frac{\hbar^2}{2m d^2} - \frac{iq\hbar^2}{m d}\right) = (E-\frac{\hbar^2 q^2}{2m}) u[i]$$
"""
...
begin
	...
	md""" 	For the numerical parameters of the simulation, we essentially only need the dimension of our hamiltonian $$N =$$ $GetN
 """
end
...
begin
	function BuildHam(q)
		...
		for i in 1:N
			x[i] = dx*i
	        H[i, i] = k*2 / dx^2 +V(x[i]) 
	        if i > 1
	            H[i, i - 1] = -k*1.0 / dx^2 + 2*im*q*k/dx
	        end
	        if i < N
	            H[i, i + 1] = -k*1.0 / dx^2 - 2*im*q*k/dx
	        end
