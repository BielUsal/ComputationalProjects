...
# ╔═╡ de585c14-4ff7-483f-b021-599751aa126d
using LinearAlgebra, OrdinaryDiffEq, Plots, PlutoUI,QuadGK,Unitful

# ╔═╡ 35579610-c146-44fa-a4d5-b1d815b9d452
begin
	import PhysicalConstants.CODATA2018:ħ,m_e
	@show hbar = ustrip(u"eV*s",ħ)
	me = ustrip(u"eV*s^2/Å^2",m_e)
	m = 0.0067*me
end

# ╔═╡ 24b272ce-522b-11ee-2440-b38df0b08d18
md"""
# Numerically Solving Repeating Finite Square Well (Inside a bigger Infinite one)
The idea in this one will be spatially discretizing our problem and doing an eigenvalue and eigenvector solution to it. We're gonna use a simple finite-diference scheme, such that we have to solve:

$$-\frac{\hbar^2}{2m} \partial_x^2 \psi + V(x)\psi = E\psi$$

Which, when we discretize will become:

$$-\frac{\hbar^2}{2m h^2} (\psi[i+1]-2\psi[i] + \psi[i-1]) + V[i]\psi[í] = E\psi[i]$$

Where $h$ is the step-size.

$$\frac{\hbar^2}{2m h^2} \left(-\psi[i+1] -  \psi[i-1] + \psi[i]\left(V[i] \frac{2mh^2}{\hbar^2}+2\right)\right) = E \psi[i]$$

$$\hat H \psi = E \psi$$

Where $H$ is a tridiagonal matrix with $diag(V[i] \frac{2mh^2}{\hbar^2}-2)$ and $ev=[1]$

A small detail is that we will consider this to be happening inside an infinite square well so that we can have boundary conditions (of the wavefunction vanishing on the edges) 
"""

# ╔═╡ fdecc5a8-5472-4baf-8b07-3dc9040841f3
begin
		DW = @bind dw NumberField(0:100, default=32)
		md"""
For physical parameters of the simulation, we only need the length of the well $$d_w =$$ $DW A 
"""
end

# ╔═╡ 8798d0b0-8764-4a47-8cd2-4d47265e5494
begin
	GetN = @bind N NumberField(0:5000,default = 1000)
	md""" 	For the numerical parameters of the simulation, we essentially only need the how many points we are considering $$N =$$ $GetN
 """
end

# ╔═╡ de1a671b-34ae-4318-930e-ad7c66a33dcb
begin
	Per = @bind Np Slider(1:10,default=1)
	md"""
	and the number of periods $$Np=$$ $Per"""
end

# ╔═╡ 7dc7e968-c053-4557-ad3e-36bb352d9b05
begin

	a = 2*dw #This is the period of our potential
	@show Gap = 35*hbar^2/(2m*a^2)
	b = Np*3dw
	function V(x) #This defines a periodic potential with period 2a
			lattice = mod(x,a) #We go from our real-coordinate x to a lattice coordinate
			if dw<lattice< 2dw
				return -Gap
			else 
				return 0
...
# ╔═╡ 26a8f1e3-69da-482c-9aa4-937768ca588d
begin
	p =plot(V,0,b,label="")
end

# ╔═╡ 22b7dfcf-5e4e-4954-882b-95e1b5a1d3a3
begin
	 x=zeros(N)
	 k = hbar^2/(2m)
	 h = (b-0)/N #Initialization of numerical things
	H = zeros(N,N)
	for i in 1:N
		x[i] = h*i 
        H[i, i] = k*2 / h^2 +V(x[i])#Defining the diagonal elements of our hamiltonian, according to the scheme at the beginning
        if i > 1
            H[i, i - 1] = -k*1.0 / h^2 #Offdiagonal elements
        end
        if i < N
            H[i, i + 1] = -k*1.0 / h^2
        end
    end
	H[1, 1] = 0.0 #Boundary conditions
    H[N, N] = 0.0
	H
end

# ╔═╡ f2c7f1cf-9752-4cf4-956c-6099cc49a2db
begin 
	E, c = eigvals(H), eigvecs(H) #We calculate the eigenvalues(Energies) and eigenvectors (wavefunctions)
	println(E)
end

# ╔═╡ 4dee84a6-0c17-4f99-b5cf-bf2fbc48212d
EBound = E[(Gap.>E.>0)] #We consider only bound states. We also disconsider the negative energy states, as I assume they are just artefacts of the numerical methods employed

# ╔═╡ 49a6804b-2d42-4906-a152-a11797eda6e7
begin
	plot(x->V(x)+Gap,0,b,label="") #We just sum a constant to the potential so its easier to visualize bound states
	hline!(EBound,label="")
end
...
# ╔═╡ 4b5b1779-6d89-438a-b42b-226c2613fff4
begin
	function Solve(Nperiods)
		x=zeros(N)
		k = hbar^2/(2m)
		b = Nperiods*3dw
		h = (b-0)/N #Initialization of numerical things
...
md"""
Here, we just do things over again defining functions to do some animations to illustrate the emmergence of the mini-bands when we take larger and larger sections of our hamiltonian. Some weird effects happen in the gifs, but these are documented problems in plotting step functions in Plots.jl, which I assume I could fix with a bit of fiddling but I think it still illustrates the point well.
"""
end

# ╔═╡ 8ab21102-4780-4eb9-b0cc-fea011473a73
begin
anim =@animate for i = 1:50
	plot(x->V(x)+Gap,0,i*3dw,label="") #We just sum a constant to the potential so its easier to visualize bound states
	hline!(Solve(i),label="")
end
...
julia_version = "1.9.1"
manifest_format = "2.0"
project_hash = "0dd3a98d18817141b325e17f58a780d376eb28ca"
