...
md"""
# Numerically Solving the Finite Square Well (Inside a bigger Infinite one)
The basic idea here is that we'll be using a (Fourier) solution basis for our problem and use Matrix Mechanics to get at the heart of things, where we will say that 

$$\psi_n(x) = \sqrt{\frac{2}{b}}\sin\left(\frac{n\pi x}{b}\right)$$

This could be done in whole different (And way faster) way, where wouldn't expand over the fourier basis, but just use normal numerical techniques to discretize the x-grid and form a tridiagonal matrix for H in which we could use normal eigenvalue techniques
"""
...
begin
	...
	a = 2*dw
	@show Gap = 30*hbar^2/(2m*a^2)
	b = 10*dw
	ψ(x,n) = sqrt(2/b)*sin(n*π*x/b)
	function V(x)
		if b/2-dw<x<b/2+dw
			return 0
		else
...
begin
	p =plot(V,0,b,label="")
	printus =plot(V,0,b,label="")
end

# ╔═╡ 8798d0b0-8764-4a47-8cd2-4d47265e5494
begin
	GetN = @bind N NumberField(0:100,default = 20)
	md""" 
	For the numerical parameters of the simulation, we essentially only need the dimension of our hamiltonian $$N =$$ $GetN
 """
end
...
begin
	...
	for i =1:N
		for j = 1:N
			Vmatrix[i,j] = quadgk(x-> ψ(x,i)*V(x)* ψ(x,j),0,b)[1]
		end
	end
	H0diag = zeros(N)
	for i =1:N 
		H0diag[i] = hbar^2*π^2*i^2/(2*m*b^2)
	end
	H0matrix = diagm(H0diag)


end

# ╔═╡ a0e2a907-be66-4771-8c07-84cc996cb504
hbar^2*pi^2/m

# ╔═╡ cb69a114-9708-4a2a-884a-a501ad6cebdb
Vmatrix

# ╔═╡ f2c7f1cf-9752-4cf4-956c-6099cc49a2db
begin 
	Hmatrix=H0matrix+Vmatrix
	E, c = eigvals(Hmatrix), eigvecs(Hmatrix)
	println(E)
	println(c)
end
...
# ╔═╡ 8d4ab7dc-cb92-48d9-9d23-18f629da3343
plot(prob,0,b,label="")

# ╔═╡ 3ceaaba7-e5d5-4972-8004-0dbb3ec594d3
begin
	plot(x->Ψ(x,c[2,:]),0,b)
end
