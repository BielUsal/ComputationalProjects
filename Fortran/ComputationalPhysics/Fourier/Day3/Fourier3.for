      ...
      Parameter(Tlim = 1.0d0)
      Parameter(dt=5d-3)
      Parameter(N=Int(Tlim/dt)) ! Tlim/dt
      Parameter(fmax = 200.d0)
      Parameter(df = 100.d-3)
      Parameter(Npf = Int(fmax/df))
      real*8 x(-N:N),t(-N:N),g
      real*8 Re(-Npf:Npf),Im(-Npf:Npf),Freq(-Npf:Npf)
      !-----------------------------------------------------------------
      open(18,file="heavi5_time_series.txt")
      open(19,file="heavi_fourier.txt")
      !-----------------------------------------------------------------
