      ...
      Parameter (N=1E5)
      Parameter (nbin=100)
      real*8 vector_gen (N),x(N),lambda,P(nbin),alpha,a,b!,vector_read (N),a
      integer Iterations(N)
      call srand(1333)
      !-----------------------------------------------------------------
      !Open up the files
      open(15, file="serieweibull.txt")
      !open(16,file="serie.txt") !read vector example
      open(17,file="histogramaweibull.txt")
      !-----------------------------------------------------------------
      lambda = 1.0d0
      alpha = 1.5d0
      do i =1, N
      	Iterations(i) = i
      	vector_gen (i) = rand(0)
      	x(i) = ((-dlog(vector_gen(i)))**(1/alpha))/lambda
